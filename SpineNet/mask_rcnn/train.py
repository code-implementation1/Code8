...
from src.blocks.mask_rcnn import MaskRCNN
...
from src.eval_utils import (
    EvalCell, COCOMeanAveragePrecision, PostProcessorMaskRCNN, prepare_masks
)
...
def main():
    ...
    model.fit(
        config.epoch_size, train_dataset, valid_dataset=val_dataset, callbacks=cb,
        dataset_sink_mode=bool(config.datasink), valid_frequency=config.eval_every
    )
...
def get_metrics(config):
    ...
    segm_post_processor = PostProcessorMaskRCNN(
        ...
        mask_postprocessor=partial(
            prepare_masks, mask_thr_binary=config.test_cfg.rcnn.mask_thr_binary
        ),
        segmentation=True
    )
    ...
    eval_metrics = {
        ...
        'bbox_mAP': COCOMeanAveragePrecision(
            ...
            mask_rcnn=False, segmentation=False
        ),
