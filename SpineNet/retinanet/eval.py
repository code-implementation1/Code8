...
from src.blocks.retinanet import RetinaNetInfer
from src.common import set_context
from src.dataset import create_custom_coco_det_dataset
from src.eval_utils import COCOMeanAveragePrecision, PostProcessorRetinaNet
...
def evaluate(ds, ckpt_path, anno_path):
    ...
    ms.load_param_into_net(model.net, param_dict_new)
    ...
    metric_bbox = COCOMeanAveragePrecision(
        ...
        post_processing=PostProcessorRetinaNet(bbox_normalization=True),
    ...
    if bbox_exist:
        ...
        for data in tqdm(
            ...
            # run net
            output = model(img_data, img_metas)
