...
class MaxIOUAssigner(nn.Cell):
    ...
    def __init__(
        ...
        self.assigned_gt_ones = ms.ops.Tensor(
        ...
        self.iou = ms.ops.IOU()
        self.max_gt = ms.ops.ArgMaxWithValue(axis=0)
        self.max_anchor = ms.ops.ArgMaxWithValue(axis=1)
        self.greaterequal = ms.ops.GreaterEqual()
    ...
    def construct(
        ...
        neg_sample_iou_mask = ms.ops.logical_and(
            ms.ops.ge(max_overlaps, self.zero_thr),
            ms.ops.less(max_overlaps, self.neg_iou_thr)
        )
        assigned_gt_inds2 = ms.ops.select(
        ...
        assigned_gt_inds3 = ms.ops.select(
            pos_sample_iou_mask,
        ...
        if self.match_low_quality:
            ...
            for j in range(self.num_gts):
                ...
                overlaps_gt_j = ms.ops.squeeze(overlaps[j:j + 1:1, ::])

                pos_mask_j = ms.ops.logical_and(
                    ms.ops.ge(max_overlaps_gt_j, self.min_pos_iou),
                    ms.ops.equal(overlaps_gt_j, max_overlaps_gt_j)
                )

                assigned_gt_inds4 = ms.ops.select(
                    pos_mask_j, self.assigned_gt_ones + j, assigned_gt_inds4
                )

        assigned_gt_inds5 = ms.ops.select(
            valid_mask, assigned_gt_inds4, self.assigned_gt_ignores
        )

        if gt_labels_i is not None:
            pos_mask = assigned_gt_inds5 > 0
            temp_gt_inds = ms.ops.select(
                pos_mask, assigned_gt_inds5 - 1, self.assigned_gt_zeros
            )
            temp_labels = gt_labels_i[temp_gt_inds]
            temp_labels = ops.reshape(temp_labels, (-1,))
            assigned_labels = ms.ops.select(
                pos_mask, ms.ops.cast(temp_labels, ms.int32),
