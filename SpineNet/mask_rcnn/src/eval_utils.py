...
class PostProcessorRetinaNet:
    """Wrapper to bbox postprocessing for RetinaNet models."""

    def __init__(
            self, bbox_normalization=True, outer_preprocessor=lambda *x, **y: x
    ):
        """Init PostProcessorRetinaNet."""
        self.bbox_normalization = bbox_normalization
        self.outer_preprocessor = outer_preprocessor

    def __call__(self, pred_bboxes, pred_labels, metas, **kwargs):
        """Run post-processing."""
        pred_bboxes, pred_labels, *_ = self.outer_preprocessor(
            pred_bboxes, pred_labels, metas
        )
        ...
        for i in range(batch_size):
            ...
            if self.bbox_normalization:
                bboxes = norm_bboxes(bboxes, meta)
...
class PostProcessorMaskRCNN:
    ...
    def __call__(
            self, pred_bboxes, pred_labels, pred_masks, pred_valid, metas,
            **kwargs
    ):
        """Run post-processing."""
        size = len(metas)
        ...
        for i in range(size):
            ...
            if self.bbox_normalization:
                bboxes_ = norm_bboxes(bboxes_, meta)
            bboxes_[::, [2, 3]] = bboxes_[::, [2, 3]] - bboxes_[::, [0, 1]] + 1
