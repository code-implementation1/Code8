...
class LossNet(nn.Cell):
    ...
    def construct(
            self, losses_bbox, losses_cls
    ):
        total_loss = 0
        for loss_bbox in losses_bbox:
            total_loss = total_loss + loss_bbox
        for loss_cls in losses_cls:
...
class WithLossCell(nn.Cell):
    ...
    def construct(
            self, x, img_shape, gt_bboxes, gt_label, gt_num
    ):
        losses_bbox, losses_cls = self._backbone(
            x, img_shape, gt_bboxes, gt_label, gt_num
        )

        return self._loss_fn(losses_bbox, losses_cls)
...
class TrainOneStepCell(nn.TrainOneStepWithLossScaleCell):
    ...
    Args:
        ...
        grad_clip (float): Clip gradients parameter. Default value is 0 (no
            clipping).
    """

    def __init__(
            self, network, optimizer, scale_sense=1, grad_clip=0.,
            accumulate_step=1
    ):
...
class TrainOneStepCellCPU(nn.Cell):
    ...
    def construct(self, x, img_shape, gt_bboxes, gt_label, gt_num):
        ...
        grads = self.grad(self.network, weights)(
            x, img_shape, gt_bboxes, gt_label, gt_num, self.sens
        )
        if self.reduce_flag:
            grads = self.grad_reducer(grads)
