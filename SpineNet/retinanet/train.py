...
from src.blocks.retinanet import RetinaNet
...
from src.eval_utils import (
    EvalCell, COCOMeanAveragePrecision, PostProcessorRetinaNet
)
...
def load_ckpt_to_network(net, checkpoint_path, finetune=False):
    ...
    if finetune:
        param_not_load = [
            'learning_rate',
            'global_step'
            'stat.bbox_head.retina_cls.weight',
            'stat.bbox_head.retina_cls.bias',
            'stat.bbox_head.retina_reg.weight',
            'stat.bbox_head.retina_reg.bias',
            'bbox_head.retina_cls.weight',
            'bbox_head.retina_cls.bias',
            'bbox_head.retina_reg.weight',
            'bbox_head.retina_reg.bias',
            'accum.bbox_head.retina_cls.weight',
            'accum.bbox_head.retina_cls.bias',
            'accum.bbox_head.retina_reg.weight',
            'accum.bbox_head.retina_reg.bias'
        ]
    else:
    ...
    for key, value in param_dict.items():
        if key in param_not_load:
            continue
        if key.startswith('zeros_.'):
            continue
        if key.startswith('accumulate_.'):
            continue
        if 'counter_' in key:
            continue
...
def main():
    ...
    experiment_name = '_'.join(
        [datetime.now().strftime('%y%m%d'), 'RetinaNet']
    )
    ...
    eval_metrics = {
        ...
        'bbox_mAP': COCOMeanAveragePrecision(
            ...
            post_processing=PostProcessorRetinaNet(
                bbox_normalization=True,
                outer_preprocessor=net.bbox_head.get_bboxes
            ),
    ...
    cb = get_callbacks(
        arch='RetinaNet', logs_dir=ckpt_save_dir / 'logs',
