...
def preprocess_coco(
    ...
    if training:
        ...
        input_data = resize_column_train(input_data, config=config)
        input_data = resize_annotation(input_data, config=config)
        input_data = crop_random(input_data, config=config)
    else:
...
def crop_random(data, config):
    """Crop image."""
    crop_size = (config.img_height, config.img_width)
    new_data = data.copy()
    img = data['image'].copy()
    segm_masks = data['segm_masks'].copy()
    bboxes = data['bboxes'].copy()
    ...
    img_shape = img.shape
    new_data['image'] = img
    new_data['resized_shape'] = img_shape[:2]
    ...
    crop_y1, crop_y2, crop_x1, crop_x2 = [
        int(x / config.mask_divider)
        for x in [crop_y1, crop_y2, crop_x1, crop_x2]
    ]
    segm_masks = segm_masks[..., crop_y1:crop_y2, crop_x1:crop_x2]

    valid_mask = (bboxes[:, 2] > bboxes[:, 0]) & (bboxes[:, 3] > bboxes[:, 1])
    valid_mask = valid_mask & data['valid'][::, 0]

    indices = np.argsort(valid_mask)[::-1]
    new_data['valid'] = valid_mask[indices][:, np.newaxis]
    new_data['bboxes'] = bboxes[indices]
    new_data['segm_masks'] = segm_masks[indices]
    new_data['labels'] = labels[indices]
