...
then
  ...
  --pre_trained=$PRETRAINED_PATH --device_target="GPU" --brief=$BRIEF --train_outputs=$TRAIN_OUT $EXTRA 2>&1 | tee train.log
fi
if [ $# -eq 5 ]
then
  python train.py --config_path=$CONFIG_PATH --train_dataset=$TRAIN_DATA --val_dataset=$VAL_DATA \
  --device_target="GPU" --brief=$BRIEF --train_outputs=$TRAIN_OUT $EXTRA 2>&1 | tee train.log
fi
