...
class RandomSampler(nn.Cell):
    ...
    def __init__(
        ...
        self.random_choice_with_mask_pos = ms.ops.RandomChoiceWithMask(
            self.num_expected_pos
        )
        self.random_choice_with_mask_neg = ms.ops.RandomChoiceWithMask(
    ...
    def construct(
        ...
        pos_index, pos_valid_index = self.random_choice_with_mask_pos(
            ms.ops.gt(assigned_gt_inds, 0)
        )

        pos_check_valid = ms.ops.cast(
            ms.ops.gt(assigned_gt_inds, 0), self.ms_type
        )
        pos_check_valid = ms.ops.reduce_sum(pos_check_valid, -1)
        pos_valid_index = ms.ops.less(self.range_pos_size, pos_check_valid)
        pos_index = pos_index * ms.ops.reshape(
            ms.ops.cast(pos_valid_index, ms.int32), (self.num_expected_pos, 1))

        pos_valid_index = ms.ops.cast(pos_valid_index, ms.int32)
        pos_index = ms.ops.reshape(pos_index, self.reshape_shape_pos)
        pos_valid_index = ms.ops.reshape(
            pos_valid_index, self.reshape_shape_pos
        )
        pos_index = pos_index * pos_valid_index

        num_sampled_pos = ms.ops.reduce_sum(
            ms.ops.cast(pos_valid_index, self.ms_type),
        )
        num_neg = self.num_expected_neg - num_sampled_pos

        if self.pos_neg_ub >= 0:
            _pos = ms.ops.maximum(1., num_sampled_pos)
            neg_upper_bound = ms.ops.cast(self.pos_neg_ub * _pos, self.ms_type)
            if neg_upper_bound < num_neg:
                num_neg = neg_upper_bound

        # Get neg index
        neg_index, neg_valid_index = self.random_choice_with_mask_neg(
            ms.ops.equal(assigned_gt_inds, 0)
        )

        sampled_neg_mask = ms.ops.less(self.range_neg_size, num_neg)
        neg_valid_index = ms.ops.logical_and(
            sampled_neg_mask, neg_valid_index
        )
        neg_index = ms.ops.reshape(neg_index, self.reshape_shape_neg)

        neg_valid_index = ms.ops.cast(neg_valid_index, ms.int32)
        neg_valid_index = ms.ops.reshape(
